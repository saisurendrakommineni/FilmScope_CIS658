...
function ForgotPassword() {
    ...
    const checkEmailExists = async () => {
        ...
        try 
        {
            const response = await fetch('https://filmscope-cis658.onrender.com/api/auth/check-email', {
                method: 'POST',
    ...
    const handlePasswordReset = async () => {
        ...
        try 
        {
            const response = await fetch('https://filmscope-cis658.onrender.com/api/auth/reset-password', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ 
                    email: email.trim(), 
